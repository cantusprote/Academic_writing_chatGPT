...
```
[Produce] → [sSb/mSb deterministic checks] → [independent verifier passes] → PASS → [게이트 원장 기록] → 다음 단계
                     │
...
- **소스 오브 트루스:** `drafts/draft_plan.md`, `data/analysis_plan.md`, `CHATGPT.md`의 사용자 제약, 그리고 현재 세션에서 사용자가 명시한 지시.
...
- **소스 오브 트루스:** 산출 섹션 자신과 인접 섹션(예: Results ↔ Discussion), `CHATGPT.md`의 Redundancy Prevention 규칙.
...
- **임무:** ChatGPT natural-language style-pass action 변환(또는 revision 재작성) 후 각 섹션이 bound 스타일과 맞는지 점검한다 — 구조·흐름, 평균 문장 길이, hedging·claim 강도, 레퍼런스 형식, 용어, 섹션별 voice/tense. lint(기계)가 못 잡는 *전체 스타일* 레이어.
- **실행 시점:** ChatGPT가 sSb/mSb로 명시적으로 실행한 deterministic lint(`lint_manuscript.py`; 필요 시 `lint_on_edit.py`) 다음. 프롬프트·출력 스키마는 `docs/verifier_prompt_templates.md`의 Style-Conformance Verifier.
...
- **기본:** semantic verifier는 가능한 경우 ChatGPT의 **higher reasoning effort**를 사용한다.
- **예외:** 사용자가 속도/비용을 우선하거나 해당 runtime에서 higher reasoning을 사용할 수 없으면 일반 reasoning으로 수행하되, deterministic checker와 source-of-truth 대조 규칙은 완화하지 않는다.
- 독립 verifier context를 사용할 때는 가능한 한 서로의 판정을 보지 않은 상태에서 같은 frozen artifact와 source-of-truth를 제공한다.
