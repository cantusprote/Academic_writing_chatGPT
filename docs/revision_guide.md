...
> 대응 전략을 정하기 전에 `paper-debate <코멘트 주제> revision`으로 ChatGPT main agent + optional sSb/mSb independent reviewer로 reviewer 대응을 설계할 수 있다 (선택). 절차: `docs/debate_protocol.md`.
...
4. **스타일 re-grounding (수정 전 필수):** 각 섹션을 고치기 전에 `docs/writing_guide.md`의 해당 섹션 규칙 + `docs/section_templates.md` + 선택한 스타일 exemplar(`Style/own/` 또는 `Style/target_journal/`) + `Style/terminology.md`를 **다시 읽고** 적용한다 — 흐릿한 기억이 아니라 실제 규칙으로. 수정 후 ChatGPT가 sSb/mSb에서 `scripts/lint_manuscript.py`를 명시적으로 실행해 용어·표기 위반을 확인한다.
...
## ChatGPT Natural-Language Actions
