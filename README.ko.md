...
**ChatGPT가 sSb/mSb를 통해 Mac의 실제 파일을 직접 다루는** 유방암 임상·중개·basic/mechanistic 연구 논문 작성 시스템입니다.

- Mac Studio 파일 작업: **sSb**
- MacBook 파일 작업: **mSb**
- 핵심 규칙: **`CHATGPT.md`**
- Upstream 기준: `grotyx/Academic_writing_c_claudecode` v1.6.3 (`e0527e2`)
- 현재 커스텀 버전: **Sim Oncology ChatGPT v0.3.0**
...
| `basic research story map 만들어줘` | `drafts/story_map.md` + claim/evidence/figure architecture 작성 |
| `mechanism이 충분한지 audit 해줘` | basic/mechanistic mechanism audit 실행 |
...
4. **Draft Plan** — `drafts/draft_plan.md` 작성·승인; substantial basic/mechanistic 연구는 `drafts/story_map.md` + Phase 3 mechanism audit까지 완료
...
- study 결과값은 `results/*.csv`에서만 가져오기
...
Wet-lab, preclinical, tumor biology, cell/molecular, organoid, animal, imaging, omics-mechanistic 연구는 `docs/basic_research_analysis_guide.md`, `docs/basic_research_guide.md`, `docs/experimental_evidence_guide.md`, `docs/basic_section_templates.md`를 사용합니다.

`story_map.md`에서 **claim → evidence → weakest link → validation → figure → boundary**를 먼저 정한 뒤 원고를 작성합니다. Full mechanism audit은 Phase 3와 Phase 6에서 수행하고, Phase 4에서는 central claim/story premise가 바뀐 경우에만 targeted re-audit합니다.

연구 **결과값**은 `results/*.csv`가 정본입니다. Dose, incubation time, replicate plan, acquisition setting 같은 experimental-design constant는 승인된 `analysis_plan.md`/Methods source에 근거합니다.
...
- `docs/basic_research_analysis_guide.md` — basic/mechanistic 분석·실험설계 override
- `docs/basic_research_checklist.md` — mechanism/reproducibility QC
- `drafts/story_map.md` — 해당 시 scientific architecture 정본
...
- PDF, 개인 profile, private style anchor는 Git에 올리지 않습니다. Local profile은 `profile.example/`을 복사해 `profile/`에 생성합니다.
